...
from database import get_db, VirtualNumber, ReceivedCode, TelegramUser
...
async def telegram_webhook(request: Request, background_tasks: BackgroundTasks, db: Session = Depends(get_db)):
    ...
    background_tasks.add_task(process_update, data, db)
    return {"ok": True}
...
async def handle_my_numbers(chat_id: str, user_id: str, db: Session):
    ...
    buttons = []
    for num in numbers:
        info = COUNTRIES.get(num.country_code.lstrip("+").lower())
        flag = "🌍"
    ...
    if not codes:
        ...
        for code in codes:
            text += (
                ...
                f"💬 Message: {code.full_message[:80]}...\n"
